...
public class NewID {
	...
	public static String solution(String new_id) {
	        ...
	        StringBuilder sbId = new StringBuilder(new_id);
	        if(sbId.length()!=0 && sbId.charAt(0)=='.') sbId.deleteCharAt(0);
	        if(sbId.length()!=0 && sbId.charAt(sbId.length()-1)=='.') sbId.deleteCharAt(sbId.length()-1);
	        if(sbId.length()==0) sbId.append("a");
	        if(sbId.length()>15) {
	        	sbId.replace(15, sbId.length(), "");
